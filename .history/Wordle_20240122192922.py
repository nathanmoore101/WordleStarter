...
from WordleGraphics import WordleGWindow, N_COLS, N_ROWS, CORRECT_COLOR, MISSING_COLOR, UNKNOWN_COLOR, KEY_COLOR 
...
def wordle():
    ...
    def enter_action(s):
        ...
        # Move to the next line after printing the guess
        ...
        if user_word.lower() not in FIVE_LETTER_WORDS:
            ...
            def compare_guess(target, guess):
                for i in range(1,6):
                if characters_array[i] == l[i]:
                    color_print(l[i], "green")
                elif guess[i] in characters_array:
                    color_print(l[i], "yellow")
                else:
                    color_print(l[i], "grey")
            print()  
