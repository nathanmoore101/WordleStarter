...
from WordleGraphics import WordleGWindow, set_square_color, N_COLS, N_ROWS, CORRECT_COLOR, PRESENT_COLOR, MISSING_COLOR, UNKNOWN_COLOR, KEY_COLOR 
...
def wordle():
    ...
    if user_word.lower() not in FIVE_LETTER_WORDS:
        ...
        for i in range(1,6):
            if target[i] == guess[i]:
                gw.set_square_color(1, guess[i], CORRECT_COLOR)
            elif guess[i] in target:
                gw.set_square_color(1, guess[i], PRESENT_COLOR)
            else:
                gw.set_square_color(1, guess[i], MISSING_COLOR)
