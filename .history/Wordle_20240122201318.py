...
def wordle():
    ...
    def enter_action(s):
        ...
        def compare_guess(target, guess):
            for i in range(1,6):
                if target[i] == guess[i]:
                    gw.set_square_color(row, col, CORRECT_COLOR)color_print(guess[i], CORRECT_COLOR)
                elif guess[i] in target:
                    color_print(guess[i], PRESENT_COLOR)
                else:
                    color_print(guess[i], MISSING_COLOR)
    ...
    gw.set_square_color(self, row, col, color)
